...
| `gallery` | Root → hall with a 3×3 grid of thin “frame” boxes on the floor. |

Fixture IDs are prefixed per kit (`default-*`, `showroom-*`, `gallery-*`) so graphs do not collide when switching kits in one session.
...
The `@diorama/examples` package is reserved for **checked-in JSON** under something like `packages/examples/scenes/`, small scripts that load scenes through `@diorama/core`, and regression assets shared by docs and tests. That work is tracked as a good-first-issue style task; contributions there are welcome.
